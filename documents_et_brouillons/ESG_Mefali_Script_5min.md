# ESG Mefali — Script de pitch (5 min, démo incluse)

*Budget : ~3 min 30 de parole + ~1 min 30 de démo • Démo placée après le Slide 3*

> ↪ = phrase de transition vers le slide suivant. Les durées sont indicatives, gardez un œil sur le total.

---

## SLIDE 1 — Couverture · `~0:15`

Bonjour, je suis Angenor N'GOUANDI, fondateur d'ESG Mefali : l'intelligence artificielle au service de la finance verte pour les PME africaines.

↪ *Pourquoi ces PME ? Parce qu'un chiffre m'a frappé.*

---

## SLIDE 2 — Le problème · `~0:40`

Moins de 10 % des financements climat en Afrique atteignent les PME. Source BAD/GCF, 2024.

Pourquoi ? Quatre murs. Un : les référentiels ESG sont complexes et souvent en anglais. Deux : un consultant coûte 5 000 à 20 000 dollars — impensable pour une PME ici. Trois : des dizaines de fonds existent, mais aucune visibilité sur lequel viser ni comment monter le dossier. Quatre : sans historique de crédit, pas de prêt bancaire, même avec d'excellentes pratiques.

↪ *Des outils existent pourtant. Pourquoi ne suffisent-ils pas ?*

---

## SLIDE 3 — La solution · `~0:25` *(puis démo)*

Là où Refinitiv ou Sustainalytics s'arrêtent au diagnostic — en anglais, à 10 000 dollars par an — ESG Mefali va jusqu'au financement. C'est un conseiller ESG virtuel en français : l'entrepreneur dialogue avec l'agent, pas de formulaires.

Plutôt que de vous le décrire, laissez-moi vous le montrer.

---

## ▶ DÉMO LIVE · `~1:30`

> **Fil rouge à dérouler à l'écran (montrez, ne lisez pas) :**
>
> 1. **Le chat** — je pose une question simple en français, l'agent répond en streaming. *(~20 s)*
> 2. **Le score** — il calcule le score ESG + l'empreinte carbone selon BCEAO / IFC / GCF. *(~20 s)*
> 3. **Le matching** — il propose les fonds verts compatibles avec ce profil. *(~20 s)*
> 4. **Le dossier** — il génère le PDF de candidature, prêt à soumettre. *(~15 s)*
> 5. **L'extension Chrome** — elle pré-remplit le formulaire directement sur le site du fonds. *(~15 s)*
>
> **Phrase de clôture de démo :** « Voilà : du diagnostic à 10 000 dollars par an, on passe à un accompagnement complet à 20 dollars par mois. »

↪ *Vous avez vu le produit. Reste la question qui compte pour vous : quelle est la taille du terrain ?*

---

## SLIDE 4 — Le marché · `~0:25`

Le terrain est immense. TAM : 14,5 milliards de dollars par an, ~100 millions de PME, +28 % par an. SAM, sur les PME francophones UEMOA/CEDEAO en secteurs verts : 2 à 4 milliards par an. SOM à 3 ans : 50 à 150 millions de financements intermédiés, via les bailleurs comme canal de distribution.

↪ *Et on monétise comment ? Notre modèle.*

---

## SLIDE 5 — Business model · `~0:25`

Freemium SaaS, trois plans : Trial gratuit 30 jours, Pro à 20 dollars par mois — notre plan recommandé — et Max à 100 dollars pour les intermédiaires et l'API.

Unit economics saines dès le départ : LTV de 504 dollars, ratio LTV/CAC de 12,6. Trajectoire : 1,51 million d'ARR à 36 mois, break-even autour du mois 18.

↪ *Voilà la mécanique. Comment on conquiert le marché concrètement ?*

---

## SLIDE 6 — Go-to-Market · `~0:20`

Trois vagues sur 36 mois. Valider : 2 marchés pilotes, 80 premiers payants. Scaler : acquisition à coût marginal via BOAD, AFD/SUNREF, GCF — 800 payants. Étendre : plan Max, API, et une Series A de 3 à 5 millions. CAC cible de 40 dollars, dont 60 % via les bailleurs.

↪ *Derrière les chiffres, il y a une raison d'être.*

---

## SLIDE 7 — Impact · `~0:20`

ESG Mefali sert cinq ODD : accès au capital, inclusion financière, économie circulaire, financement climat, et partenariats. À 3 ans : 10 000 PME accompagnées, 100 millions de dollars débloqués, 8 pays UEMOA couverts.

↪ *Et tout ça n'est pas une idée — vous venez de le voir tourner.*

---

## SLIDE 8 — Traction & roadmap · `~0:20`

Le MVP que je viens de vous montrer est livré : chat IA, scoring multi-référentiel, calculateur carbone, matching sur 10 fonds, extension Chrome, déploiement Docker.

La roadmap : pilote 10 PME en Côte d'Ivoire au T1 2026, partenariats BOAD au T2, lancement UEMOA sur 8 pays au T3, puis toute l'Afrique francophone en 2027.

↪ *Permettez-moi de conclure par le « pourquoi ».*

---

## SLIDE 9 — Clôture · `~0:20`

J'ai conçu ESG Mefali parce que j'ai vu des PME parfaitement viables se faire refuser un financement faute d'outils accessibles. Notre conviction tient en une ligne :

**La finance verte, accessible à tous.**

Merci.

*Pour échanger : Angenor99@gmail.com — linkedin.com/in/angenor-ngouandi*

---

### Conseils de timing pour tenir les 5 min
- **La démo est le poste à risque.** Préparez un scénario figé (même question, même PME de test) et entraînez-vous 2-3 fois chrono en main. Si elle dérape, ayez une vidéo/capture de secours.
- Si vous débordez, le Slide 8 est le plus compressible (la démo l'a déjà prouvé) : une phrase suffit.
- Gardez 5-10 secondes de silence après « accessible à tous » avant le « Merci » — ça pose la phrase.
