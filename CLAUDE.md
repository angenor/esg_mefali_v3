...
- **F12** — voir Fondations.
- **F13** : voir « Module ESG ».
- **F18 — Widgets interactifs chat** (mig. 018) : tool `ask_interactive_question` (4 variantes qcu/qcm/qcu_justification/qcm_justification) injecté dans 7 noeuds. Table `interactive_questions` satellite (aucune modif autres). Marker SSE `<!--SSE:{"__sse_interactive_question__":true,...}-->`. 2 events SSE (`interactive_question`, `interactive_question_resolved`) + endpoints `POST /chat/interactive-questions/{id}/abandon`, `GET /chat/conversations/{id}/interactive-questions`. Helper `WIDGET_INSTRUCTION` injecte dans 6 prompts modules + chat_node. Composants Vue : `InteractiveQuestionHost`, `SingleChoiceWidget`, `MultipleChoiceWidget`, `JustificationField`, `AnswerElsewhereButton` (ARIA radiogroup/checkbox). 1 question pending max/conversation. Justification ≤ 400 c. **Fix bug `account_id` NULL (2026-05-08)** : helpers `_resolve_account_id(db, config, conversation_id)` (priorité `config['configurable']['account_id']` puis fallback `SELECT FROM conversations`) + `_resolve_assistant_message_id(config)` ajoutés dans `app/graph/tools/interactive_tools.py` ; tool `ask_interactive_question` et helper `_persist_widget_question` (9 widgets F10) propagent désormais `account_id` + `assistant_message_id` à l'INSERT pour respecter la contrainte NOT NULL F02 mig. 019. `_extract_context` rendu async (6-tuple). Erreur claire « account_id du tenant introuvable » retournée au LLM si non résolvable, plus jamais de 500 sur violation NOT NULL.
- **F11** — voir Fondations.
...
- **F25 — Catalogue admin financement vert** (sans migration, lecture F01/F06/F07/F04, feature 044) : route `/api/admin/catalog/*` (router parent `app/modules/admin/catalog_router.py` avec `Depends(get_current_admin)`) regroupant 3 endpoints — `GET /summary` (4 COUNT(*) GROUP BY publication_status + COUNT actif/expiré liaisons), `GET /fund-intermediaries` + `GET /fund-intermediaries/{id}` (sous-router `fund_intermediaries_router.py`, id composite `"{fund_id}:{intermediary_id}"`, pagination conditionnelle FR-013a seuil 2000, filtres q/status/fund_id/intermediary_id/sort, jointures Fund.name/Intermediary.name), `GET /export?tab=...` (`StreamingResponse` text/csv UTF-8+BOM, filename `catalog-{tab}-YYYYMMDD.csv`, validation combinaisons incompatibles 422). Helper `catalog_helpers.compute_has_incoherence` (D5 research : publié sans source F01, ou liaison accredited_to dépassée). Service `catalog_export_service.stream_csv` (4 mappers colonnes, BOM préfixe, ligne-par-ligne via `db.stream`). Enrichissement sérialisation des routers F09 existants (`funds_router`, `intermediaries_router`, `offers_router`) pour exposer `version`, `valid_from`, `valid_to`, `superseded_by`, `source_id`, `has_incoherence`. Schemas Pydantic v2 strict dans `app/schemas/admin_catalog.py` (`CatalogSummary`, `CatalogTabCounts`, `CatalogFundIntermediariesCounts`, `FundIntermediaryRow`/`Detail`, `PaginatedListResponse[T]`, `SourceRef`, `CatalogExportTab` Literal). Frontend : types miroir `frontend/app/types/adminCatalog.ts`, store Pinia `stores/adminCatalog.ts` (4 onglets isolés, filtres + pagination par onglet, volatile sans persistance), composable `composables/useAdminCatalog.ts` (debounce 300 ms sur `q`, redirection /dashboard sur 403, normalisation des 2 formats backend `paginated` + F09 `limit`), 6 composants `components/admin/catalog/*` (`CatalogTabs` ARIA tablist + nav clavier flèches, `CatalogToolbar` recherche/statut/tri/bouton export, `CatalogTable` colonnes configurables + pagination conditionnelle, `CatalogEmptyState` 2 modes + bouton reset, `CatalogIncoherenceBadge` amber + tooltip, `FundIntermediaryDetail` avec MoneyDisplay + SourceLink + SuccessorBanner, `SuccessorBanner` lien NuxtLink versioning F04). Pages `pages/admin/catalog/index.vue` (shell layout admin + middleware admin) + `fund-intermediaries/[id].vue` + 3 wrappers `funds|intermediaries|offers/[id].vue` (redirect vers `/admin/{entity}/{id}` existant). Entrée sidebar `layouts/admin.vue` « Catalogue fonds » déjà câblée. **Tests** : 25 backend pytest (84% coverage modules nouveaux) + 32 frontend Vitest + 2 E2E Playwright (access + export). **Lecture seule** (FR-013) : aucune mutation déclenchée depuis `/admin/catalog`. Catalogue référentiel global → hors F03 audit log (assumption spec).
...
- **Round-trip Alembic** `up/down/up` validé sur PostgreSQL pour toutes les migrations 019-042 (044 idempotente : `ALTER TYPE ADD VALUE IF NOT EXISTS`, downgrade no-op car PG ne supporte pas DROP VALUE).
- **Bug fix 2026-05-20** — 3 bugs corrigés : (1) `app/modules/financing/router.py:241` — `profile.annual_revenue` n'existe pas, remplacé par `profile.annual_revenue_xof` (colonne legacy F04 BigInteger XOF) avec fallback `is not None` pour profils sans CA renseigné ; (2) mig. **044** `add_carbon_to_report_type_enum` — ajoute la valeur PG `'carbon'` à `report_type_enum` (l'enum Python F21 contenait `carbon` sans migration côté BDD, d'où 500 sur `GET /api/reports/?type=carbon`) ; (3) `app/graph/tools/carbon_tools.py::save_emission_entry` — quand le facteur d'émission n'est pas trouvé, le payload d'erreur enrichi (`did_you_mean` via difflib + fallback substring alphanumérique normalisé, `available_categories_sample` ≤ 20, `instruction_to_llm`) force le LLM à retry avec une clé valide (`electricite` → `electricity`, `R-404A` → `refrigerant_r404a`, `gasoil` → `fuel_gasoline`) au lieu de basculer en calcul mental. Docstring du tool enrichie avec les subcategories valides les plus courantes par catégorie.
...
## Active Technologies
- Python 3.12 (backend), TypeScript 5.x strict (frontend) + FastAPI, SQLAlchemy async, Pydantic v2 (backend) ; Nuxt 4, Vue 3 Composition API, Pinia, TailwindCSS (frontend) (044-admin-catalog-financing)
- PostgreSQL 16 (tables existantes `funds`, `intermediaries`, `offers`, `fund_intermediaries`, `sources`) ; aucune nouvelle table ni migration (044-admin-catalog-financing)
- Python 3.12 (backend), TypeScript 5.x strict (frontend) + FastAPI, SQLAlchemy async, Pydantic v2, Alembic, LangGraph (>=0.2), LangChain (>=0.3), WeasyPrint, Jinja2, matplotlib (backend) ; Nuxt 4, Vue 3 Composition API, Pinia, TailwindCSS, Chart.js (frontend) (047-evaluation-esg-projet)
- PostgreSQL 16 + pgvector. **2 nouvelles tables** (`project_esg_assessments`, `project_esg_criterion_responses`) + 1 migration Alembic 047. Réutilisation des tables existantes `projects` (F045), `referentials` + `criteria` (F13), `sources` (F01), `audit_log` (F03). (047-evaluation-esg-projet)

## Recent Changes
- 044-admin-catalog-financing: Added Python 3.12 (backend), TypeScript 5.x strict (frontend) + FastAPI, SQLAlchemy async, Pydantic v2 (backend) ; Nuxt 4, Vue 3 Composition API, Pinia, TailwindCSS (frontend)
