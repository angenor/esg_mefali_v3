...
- **Sélection tools (F048)** : base = **union** `GLOBAL_WHITELIST ∪ MODULE_TOOL_MAPPING[node] ∪ PAGE_TOOL_MAPPING[slug]`, **intersectée avec le catalogue réel du nœud** (`& available_names`). Troncature priorisée : whitelist > tools du nœud (intention) > tools de page. ⚠️ Un tool de `GLOBAL_WHITELIST` n'est exposé que s'il figure AUSSI dans le catalogue (`all_tools`) du nœud appelant ET dans son `ToolNode` (exécutabilité) — la whitelist seule ne suffit pas à rendre un tool transverse (cf. F047/051).
- **`MAX_TOOLS_PER_TURN = 36`** (et NON 14 — la borne a évolué 14→22→26→29→31→36 ; cf. commentaire `tool_selector_config.py`). `GLOBAL_WHITELIST` = **21 tools** : `ask_interactive_question`, `trigger_guided_tour` ; F01 `cite_source`/`search_source`/`flag_unsourced` ; F12 `recall_history` ; F10 widgets `ask_yes_no`/`ask_select`/`ask_number`/`ask_date`/`ask_date_range`/`ask_rating` ; F20 `search_resources`/`get_resource_content`/`recommend_resources_for_user` ; F047 6 tools ESG-projet (`create`/`save`/`finalize`/`get`/`list_project_esg_assessment(s)` + `generate_project_esg_report`).
- **051 — Découverte/statut depuis le chat flottant** : `list_applications` + `get_application_checklist` (LECTURE SEULE, scopés `user_id` anti-IDOR) ajoutés à la **base du nœud `chat`** (`MODULE_TOOL_MAPPING["chat"]` + `PAGE_TOOL_MAPPING["chat_global"]` + catalogue chat_node + ToolNode chat via `APPLICATION_STATUS_TOOLS`), **pas** à `GLOBAL_WHITELIST` (qui pousserait profile_projects/financing à 37 > borne). `get_esg_assessment_chat` (score ESG) + `get_user_dashboard_summary` (avancement) y étaient déjà. Permet « génère mon score ESG » / « où en est mon dossier ? » depuis n'importe quelle page. Sur pages riches (/financing, /carbon, /esg, /profile) la base chat dépasse 36 → troncature **page-tier bénigne** (tools du nœud chat toujours préservés).
